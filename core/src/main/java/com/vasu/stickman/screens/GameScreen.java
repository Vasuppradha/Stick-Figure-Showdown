package com.vasu.stickman.screens;
...
import com.vasu.stickman.objects.BloodPool;
import com.vasu.stickman.objects.BloodSplatter;
...
public class GameScreen implements Screen, InputProcessor {
    ...
    private void createBlood(){
        ...
        //set the current blood splatter indexes to start of the arrays
        currentPlayerBloodSplatterIndex = 0;
        currentOpponentBloodSplatterIndex = 0;

        //initialize the blood polls
    ...
    private void resumeGame(){
        ...
        //resume game sounds if it is enabled
    ...
    private void loseRound(){
        //player lose the round and opponent wins
        game.player.lose();
        game.opponent.win();
        roundsLost++;
    ...
    public void render(float delta) {
        ...
        //set the sprite batch and the shape renderer to use the camera
        ...
        game.batch.draw(frontRopesTexture,0,0,frontRopesTexture.getWidth()*GlobalVariables.WORLD_SCALE, frontRopesTexture.getHeight()*GlobalVariables.WORLD_SCALE);
        ...
        //if the game is over , draw the gam over state
    ...
    private void renderFighters(){
        //use the y coordinates of the fighter's positions to determine which figure to draw first
    ...
    private void renderHUD(){
        ...
        smallFont.draw(game.batch, text, viewport.getMaxWorldWidth() - HUDMargin, viewport.getWorldHeight() - HUDMargin, 0, Align.right, false);
            ...
            if(areWithinContactDistance(game.player.getPosition(), game.opponent.getPosition())){
                if(game.player.isAttackActive()){
                    ...
                    if (game.opponent.isBlocking()){
                        ...
                        //if opponent isn't blocking, play the hit sound
                    ...
                    if(game.opponent.isAttackActive()){
                        ...
                        if (game.player.isBlocking()){
                            ...
                            //if player isn't blocking, play the hit sound
                            game.audioManager.playSound(Assets.HIT_SOUND);
    ...
    private void opponentAiMakeContactDecision(){
        ...
        if (game.player.isAttacking()){
            ...
            //if player isn't attacking, determine whether to attack player or move away from the player
    ...
    public void opponentAiMoveAwayFromPlayer(){
    ...
    public void resize(int width, int height){
        //update the viewport with new screen size
    ...
    @Override
    public void pause() {
        //if the game is running, then pause
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        if (gameState == GameState.RUNNING){
            if (pauseButtonSprite.getBoundingRectangle().contains(position.x, position.y)){
                ...
                //if the round id starting and the screen has been touched, skip the start round delay
                roundStateTime = START_ROUND_DELAY;
            } else if (roundState == RoundState.ENDING) {
            ...
            if (gameState == GameState.GAME_OVER && playAgainButtonSprite.getBoundingRectangle().contains(position.x, position.y)) {
                //if the game ios over and the play again button has been pressed, start the game from the beginning
                startGame();
