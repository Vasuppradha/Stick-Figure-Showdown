package com.vasu.stickman.objects;
...
public class Fighter {
    ...
    //factor to decrease change if a fighter gets hit while blocking
    ...
    public void render(SpriteBatch batch){
        ...
        batch.setColor(Color.valueOf(color));
        batch.draw(currentFrame, position.x, position.y,currentFrame.getRegionWidth() * 0.5f * GlobalVariables.WORLD_SCALE,0, currentFrame.getRegionWidth() * GlobalVariables.WORLD_SCALE, currentFrame.getRegionHeight()* GlobalVariables.WORLD_SCALE, facing, 1,0);
        batch.setColor(1, 1, 1, 1);
    }

    public void update(float deltaTime){
        //increment the state time by the delta time
        stateTime += deltaTime;
        ...
        }else if((state == State.PUNCH && punchAnimation.isAnimationFinished(stateTime)) || (state == State.KICK && kickAnimation.isAnimationFinished(stateTime))|| (state == State.HURT && hurtAnimation.isAnimationFinished(stateTime))){
    ...
    public boolean isAttackActive(){
        //the attack is only active if the fighter has not yet made contact
        //and the attack has not just started or is almost finished
        if(hasMadeContact()){
            return false;
